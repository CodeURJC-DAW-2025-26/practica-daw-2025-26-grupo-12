(() => {
  document.addEventListener("DOMContentLoaded", () => {
    const searchForm = document.getElementById("admin-tournament-search-form");
    const searchInput = document.getElementById("admin-tournament-search");
    const tableBody = document.getElementById("admin-tournament-table-body");
    const resultCounter = document.getElementById("admin-tournament-counter");
    const loadMoreBtn = document.getElementById("show-more-admin-tournaments-btn");
    ...
    const initialMeta = document.querySelector(".admin-tournament-meta-row");
    if (initialMeta) {
    ...
    const parseRowsPayload = (html) => {
      ...
      const metaRow = tempBody.querySelector(".admin-tournament-meta-row");
      if (!metaRow) {
        throw new Error("Missing admin tournament metadata row");
      }
    ...
    const loadTournaments = async (query, page = 0, append = false) => {
      ...
      try {
        const url = new URL("/admin/tournaments/page", window.location.origin);
        if (query) {
